...
def get_best_model(args, directory_path):
    ...
    try:
        ...
        return model, int(best_model_file.split("_")[-1])
...
def evaluate(args, model, dataset, compute_evaluation_metrics_flag, result_dict=None):
    """
    ...
    test_dataloader = DataLoader(graph_set, num_workers=8)
    ...
    print(
        f"Loading center {os.path.join(args.center_path, args.graph_type, 'center.pt')}")
    center = torch.load(os.path.join(
        args.center_path, args.graph_type, 'center.pt')).to(args.device)
    print(
        f"Loading path {os.path.join(args.radius_path, args.graph_type, f'radius_{step}.pt')}")
    radius = torch.load(os.path.join(
        args.radius_path, args.graph_type, f'radius_{step}.pt')).to(args.device)
    total_time = 0.0
    score_list = []
    labels_list = []
    inference_time_list = []
...
if __name__ == '__main__':
    ...
    model, step = get_best_model(args, os.path.join(
